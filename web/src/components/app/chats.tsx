...
const sampleMessages: Message[] = [
  {
    id: 'm1',
    sender: 'them',
    content: 'Can we sync later today?',
    time: '12:35',
  },
  {
    id: 'm2',
    sender: 'you',
    content: 'Sure. 6 PM work for you?',
    time: '12:37',
    status: 'delivered',
  },
  {
...
function StatusDot({ status }: { status?: Conversation['status'] }) {
  ...
  return (
    <span
      className={`inline-block h-2.5 w-2.5 rounded-full ring-2 ring-(--surface) ${status ? palette[status] : 'bg-(--line)'}`}
    />
  )
}
      ...
      <section className="flex flex-1 h-full overflow-hidden">
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      <p
                        className={`truncate text-sm font-semibold transition-colors ${active ? 'text-(--sea-ink)' : 'text-(--sea-ink)'}`}
                      >
                        {c.name}
                      </p>
                      <span className="ml-auto text-[11px] text-(--sea-ink-soft)">
                        {c.time}
                      </span>
                    </div>
                    <div className="flex items-center gap-2">
                      <p className="truncate text-xs text-(--sea-ink-soft) opacity-90 group-hover:opacity-100">
                        {c.snippet}
                      </p>
                      {c.unread ? (
        ...
        <div className="flex min-w-0 flex-1 flex-col bg-(--foam) dark:bg-black">
          <header className="flex h-18 items-center gap-3 border-b border-(--line) bg-(--surface-strong) dark:bg-black px-4 py-3 sm:px-6">
            ...
            <div className="min-w-0">
              <h2 className="truncate text-base font-semibold text-(--sea-ink)">
                {selected.name}
              </h2>
              <div className="flex items-center gap-1.5 text-xs text-(--sea-ink-soft)">
                <div
                  className={`h-1.5 w-1.5 rounded-full ${selected.status === 'online' ? 'bg-emerald-500' : 'bg-gray-400'}`}
                />
                ...
                return (
                  ...
                  >
                    ...
                    >
                      ...
                      <div
                        className={`mt-1 flex items-center justify-end gap-1 text-[10px] ${outgoing ? 'text-white/80' : 'text-(--sea-ink-soft)'}`}
                      >
                        <span>{m.time}</span>
                        {outgoing && m.status ? (
                          <CheckCheck
                            className={`size-3.5 ${m.status === 'read' ? 'text-white' : 'text-white/60'}`}
                          />
                        ) : null}
                      </div>
                    </div>
        ...
        <aside className="hidden w-70 flex-col border-l border-(--line) bg-(--surface-strong) dark:bg-black px-6 py-6 lg:flex">
          <div className="flex flex-col items-center">
            ...
            <h3 className="mt-3 text-lg font-bold text-(--sea-ink)">
              {selected.name}
            </h3>
          ...
          <div className="mt-8 space-y-4">
            <div className="rounded-xl border border-(--line) bg-(--surface) p-4">
              <p className="text-[10px] uppercase tracking-wider text-(--sea-ink-soft) font-bold mb-2">
                Status
              </p>
              <div className="flex items-center gap-2">
            ...
            <div className="rounded-xl border border-(--line) bg-(--surface) p-4">
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs text-(--sea-ink-soft)">
                  Messages are secured with session keys.
                </p>
              </div>
            </div>
            ...
            <div className="rounded-xl border border-(--line) bg-(--surface) p-4">
              <div className="flex items-center justify-between mb-2">
                <p className="text-[10px] uppercase tracking-wider text-(--sea-ink-soft) font-bold">
                  Shared Media
                </p>
                <button className="text-[10px] text-(--lagoon) hover:underline font-medium">
                  View All
                </button>
              </div>
